#!/usr/bin/env python3
"""Same as infer_policy.py (native GLFW viewer, unchanged), plus an
autonomous walk<->splits state machine on top: alternates the walking
policy (random forward speed + slight random turn each time WALK is
entered) with the splits skill (one-way descend-and-hold -- it doesn't
stand back up on its own, deliberately left that way for v1 so we can see
what actually happens on the walk<->splits handoff), on a randomized
timer with a 50/50 coin flip at each dwell boundary deciding stay-vs-switch.
Any manual velocity keypress (arrows/A/E/space) pauses the automatic
switching until `--idle-timeout` seconds pass with no further keypresses.

Built for running natively (no Docker) on an M-series Mac, where the
native viewer has a real display/GPU to attach to. infer_policy.py itself
is left untouched -- this is a separate copy."""
...
class PolicyInference:
    def __init__(self, model, data, walking_onnx_path=None, action_scale=1.0,
                 ...
                 splits_onnx_path=None, splits_duration=4.0):
        self.model = model
        self.data = data
        ...
        for name, path, duration in (
            ...
            ("splits", splits_onnx_path, splits_duration),
        ):
            if not path:
                continue
            if not self.new_cmd_obs:
    ...
    def trigger_behavior(self, name, duration=None):
        """Start an episodic behavior (kick_left / kick_right / roulade / splits).

        The behavior policies were trained to run from a standing start with an
        all-zero command, so triggering is a session swap; a timer hands
        control back to walking/standing afterwards. `duration` overrides the
        fixed --kick-duration/--roulade-duration/etc default for this one
        call (used by the walk<->splits state machine to randomize dwell
        time per entry); None keeps the constructor default.

        Note: splits (v1) ends the split, not standing -- it was only ever
        trained to descend and hold, so "auto-return to walking" here means
        a straight policy swap from the split pose, not a trained stand-up.
        """
...
class WalkSplitsStateMachine:
    """Autonomous WALK<->SPLITS switcher with keyboard override.

    WALK: a fresh random (lin_vel_x, lin_vel_y) each time it's (re-)entered,
    held for a random dwell. SPLITS: policy.trigger_behavior("splits", ...)
    for a random dwell -- that call already owns its own auto-return-to-
    walking timer (update_behavior), so this class just reacts once it sees
    behavior_mode flip back to None.

    At each dwell boundary a fair coin flip decides stay (re-roll the same
    kind of state) vs switch (go to the other one) -- so either state can
    self-loop indefinitely, matching "toss a coin" rather than a strict
    alternation.

    Any manual velocity keypress (mark_manual_key) suspends the automatic
    dwell/coin logic -- update() just returns early -- until idle_timeout
    seconds pass with no further keypresses, at which point it resumes with
    a fresh dwell timer but WITHOUT clobbering whatever velocity you last
    set (only re-randomizes on the next natural dwell expiry or a splits
    return). Splits returning to walking always re-randomizes regardless
    of manual/auto, since splits held vel_cmd at zero throughout and
    resuming with zero would just stand still.
    """

    def __init__(self, policy, walk_dwell_range, splits_dwell_range,
                 vel_x_range, vel_y_range, idle_timeout):
        self.policy = policy
        self.walk_dwell_range = walk_dwell_range
        self.splits_dwell_range = splits_dwell_range
    ...
    def _enter_walk(self, now, randomize):
        ...
        dwell = random.uniform(*self.walk_dwell_range)
        self._dwell_end = now + dwell
        print(f"[auto] WALK for {dwell:.1f}s "
              f"(vx={self.policy.vel_cmd[0]:.2f}, vy={self.policy.vel_cmd[1]:.2f})")

    def _enter_splits(self, now):
        dwell = random.uniform(*self.splits_dwell_range)
        self.policy.trigger_behavior("splits", duration=dwell)
        self._dwell_end = now + dwell
        print(f"[auto] SPLITS for {dwell:.1f}s")

    def update(self, now):
        behavior_mode = self.policy.behavior_mode
        just_returned_from_splits = (
            self._prev_behavior_mode == "splits" and behavior_mode is None
        )
        self._prev_behavior_mode = behavior_mode
        if just_returned_from_splits:
            self._enter_walk(now, randomize=True)
            return  # fresh dwell just started; nothing else to do this tick
...
def main():
    ...
    parser.add_argument("--splits", type=str, default=None,
                         help="Path to splits skill policy ONNX (one-way descend-and-hold, no "
                              "trained stand-up). Requires --new-cmd-obs. With --walking also "
                              "given, drives the autonomous walk<->splits state machine; with "
                              "only --splits, it's just loaded inert (nothing triggers it).")
    ...
    parser.add_argument("--splits-dwell", type=float, nargs=2, default=[3.0, 5.0], metavar=("MIN", "MAX"),
                         help="Auto state machine: SPLITS dwell time range in seconds (default: 3.0 5.0)")
    ...
    if (args.kick_left or args.kick_right or args.roulade or args.splits) and not args.new_cmd_obs:
        parser.error("--kick-left/--kick-right/--roulade/--splits policies use the unified 13D command obs (61D); add --new-cmd-obs")
    if (args.kick_left or args.kick_right or args.roulade or args.splits) and args.roller:
        parser.error("kick/roulade/splits policies are trained on the walking robot, not the roller model")
    ...
    for _name in policy.behavior_sessions:
        if _name in _behavior_keys:
            ...
            print(f"{_name} policy: loaded  (triggered by the auto walk<->splits state machine)")
    print(f"Active policy: {policy.current_policy}")
    print("Close viewer window to exit")
    print()

    state_machine = None
    if policy.walking_session and "splits" in policy.behavior_sessions:
        state_machine = WalkSplitsStateMachine(
            policy,
            walk_dwell_range=tuple(args.walk_dwell),
            splits_dwell_range=tuple(args.splits_dwell),
            vel_x_range=tuple(args.vel_x_range),
            vel_y_range=tuple(args.vel_y_range),
            idle_timeout=args.idle_timeout,
        )
        print(f"Auto state machine: ON  (WALK {args.walk_dwell[0]:.0f}-{args.walk_dwell[1]:.0f}s / "
              f"SPLITS {args.splits_dwell[0]:.0f}-{args.splits_dwell[1]:.0f}s, "
              f"50/50 coin flip at each boundary; any velocity key pauses it for "
              f"{args.idle_timeout:.0f}s idle before it resumes)")
        print()
    ...
    def handle_key(key):
        ...
        # These are exactly the keys/branches below that end up calling
        # policy.set_vel_cmd (head/body-pose mode intercept up/down/left/
        # right/a/e/space first and never touch vel_cmd, so they're
        # deliberately excluded here).
        if (state_machine is not None and key in ("up", "down", "left", "right", "a", "e", " ")
                and not policy.head_mode and not policy.body_pose_mode):
