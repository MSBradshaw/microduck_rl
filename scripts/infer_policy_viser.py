#!/usr/bin/env python3
"""Same as infer_policy.py, but rendered through viser (browser) instead of
MuJoCo's native GLFW viewer -- for headless Docker containers with no
display attached (mujoco.viewer.launch_passive needs a real window; this
streams the scene to a browser tab on --port instead, the same way mjlab's
own `play --viewer viser` does). Keyboard control is unaffected: keys are
still read from the terminal (see TerminalInput), not the browser.

Keep infer_policy.py itself untouched -- this is a separate copy so the
native-viewer path (e.g. on an M-series Mac with a real display) still
...
def main():
    ...
    parser.add_argument("--port", type=int, default=8080, help="Viser server port (open http://localhost:PORT in a browser) (default: 8080)")
    ...
    print("Ctrl+C or press Q to exit")
    print()
    ...
    print("\nKeyboard controls (type in THIS terminal — the browser viewer doesn't capture keys):")
    ...
    print(f"\nViser viewer running — open http://localhost:{args.port} in a browser\n")
