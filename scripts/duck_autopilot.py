#!/usr/bin/env python3
"""v2 of the autonomous state machine (see infer_policy_statemachine.py for
v1, left untouched -- v1's checkpoints (Velocity, Splits v1) exist today,
v2's (VelStand, SitStand, SplitsCycle, StandUp, Roulade, BallKick) are
still training as of 2026-09-04, see
~/microduck-results/batch1-walking-2026-09-03/README.md).

Rotates through whichever of {walk, sit, split, roulade, kick_left,
kick_right} were actually loaded via CLI flags -- see the AutoStateMachine
class docstring near the bottom for the full lifecycle of each kind
(continuous / flag-toggled posture / one-shot-with-timer), and why "sit"
specifically chains into the dedicated `standup` skill instead of
sitstand's own built-in self-rise. Native GLFW viewer, same as
infer_policy.py (untouched) -- built for the M-series Mac.
"""
...
class PolicyInference:
    def __init__(self, model, data, walking_onnx_path=None, action_scale=1.0,
                 ...
                 splits_cycle_onnx_path=None,
                 standup_onnx_path=None, standup_duration=3.0):
        self.model = model
        self.data = data
        ...
        # Splits-cycle: same flag-commanded posture mechanism as sitstand
        # (twist[0] = 0/1, SAME policy holds/rises), just a different target
        # pose and loaded as its own independent session -- sit and split
        # can both be available at once, each toggled separately.
        self.split_session = None
        self.split_mode = False
        if splits_cycle_onnx_path:
            if not self.new_cmd_obs:
                raise ValueError(
                    "--splits-cycle policies use the unified 13D command obs (61D); run with --new-cmd-obs"
                )
            print(f"\nLoading splits-cycle policy from: {splits_cycle_onnx_path}")
            self.split_session = ort.InferenceSession(splits_cycle_onnx_path)
            sc_input_shape = self.split_session.get_inputs()[0].shape
            print(f"Splits-cycle policy input shape: {sc_input_shape}")
        ...
        for name, path, duration in (
            ...
            ("standup", standup_onnx_path, standup_duration),
        ):
            if not path:
                continue
            if not self.new_cmd_obs:
    ...
    def _update_command(self):
        ...
        if self.new_cmd_obs:
            ...
            elif self.current_policy == "split":
                # Same flag mechanism as sitstand, different target pose.
                cmd[0] = 1.0 if self.split_mode else 0.0
    ...
    def trigger_behavior(self, name, duration=None):
        """Start an episodic behavior (kick_left / kick_right / roulade / standup).

        The behavior policies were trained to run from a standing start (or,
        for standup, a seated one -- see toggle_sit_via_standup) with an
        all-zero command and end standing, so triggering is a session swap; a
        timer hands control back to walking/standing afterwards. `duration`
        overrides the constructor's fixed default for this one call (used by
        AutoStateMachine to randomize dwell per entry); None keeps it.

        Note the sit_mode guard below is intentionally skipped for "standup":
        that's the ONE behavior meant to trigger FROM a seated pose (its only
        trained entry condition) -- see AutoStateMachine._exit_posture, which
        clears sit_mode itself immediately before calling this so sitstand's
        own self-rise never gets a tick to run.
        """
        ...
        if self.sit_mode:
            ...
            return
        if self.split_mode:
            print(f"Cannot start {name} while split (exit split first)")
            return
        if self.slope_mode:
        ...
        self.behavior_time_left = duration if duration is not None else self.behavior_durations[name]
        self.vel_cmd = np.zeros(3, dtype=np.float32)
    ...
    def toggle_sit(self):
        ...
        if self.behavior_mode is not None:
            ...
            return
        if self.split_mode:
            print("Cannot sit while split (exit split first)")
            return
    ...
    def toggle_split(self):
        """Toggle the splits-cycle posture flag (0=stand, 1=split).

        Mirrors toggle_sit()'s sitstand branch exactly -- same flag-commanded
        mechanism (the SAME policy holds and rises, trained response to a
        flag flip is a gentle glide), different target pose, own independent
        session/flag so sit and split can both be loaded without conflict
        (mutually exclusive at any instant, guarded below).
        """
        if self.split_session is None:
            print("Split unavailable: no --splits-cycle policy loaded")
            return
        if self.ground_pick_mode:
            print("Cannot split during ground pick")
            return
        if self.behavior_mode is not None:
            print(f"Cannot split during {self.behavior_mode}")
            return
        if self.sit_mode:
            print("Cannot split while sitting (exit sit first)")
            return
        self.split_mode = not self.split_mode
        if self.split_mode:
            self.vel_cmd = np.zeros(3, dtype=np.float32)
            self.current_policy = "split"
            self.ort_session = self.split_session
            print("Split: ON (flag=1; toggle again to stand up)")
        else:
            # Stay on the split session — it stands up itself (flag → 0),
            # same reasoning as sitstand's own self-rise.
            print("Split: OFF → splits-cycle policy standing up (flag=0)")
        self._update_command()
...
# One-shot behaviors: policy.trigger_behavior owns the whole lifecycle
# (including auto-return to walking) via policy.update_behavior's timer.
_ONE_SHOT_NAMES = ("roulade", "kick_left", "kick_right", "standup")
# Flag-toggled postures: held indefinitely until explicitly flipped back;
# AutoStateMachine owns the dwell timer itself (no auto-return signal).
_POSTURE_NAMES = ("sit", "split")


class AutoStateMachine:
    """Autonomous multi-skill switcher with keyboard override (v2).

    Rotates through whichever of {walk, sit, split, roulade, kick_left,
    kick_right} were actually loaded via CLI flags -- built dynamically in
    main() from whatever policy sessions ended up non-None, so it degrades
    gracefully as checkpoints land (see
    ~/microduck-results/batch1-walking-2026-09-03/README.md for what's still
    training as of 2026-09-04).

    "walk" is the hub: every exit -- a one-shot finishing, a posture
    standing back up -- lands back on "walk" before the next dwell/coin
    decision runs, so no trick chains directly into another trick (same
    handoff v1 used for splits->walk). Three different lifecycles depending
    on the pool member's kind:

    - "walk": continuous, twist-commanded. Entering samples a fresh random
      (lin_vel_x, lin_vel_y); dwell is a plain timer. At the dwell boundary,
      a fair coin decides: stay (fresh walk dwell) vs switch (uniformly pick
      one of the OTHER loaded pool members). This is the only place that
      coin fires -- postures and one-shots have their own logic below, not
      this generic switch.

    - "sit" / "split" (postures): flag-commanded via policy.toggle_sit() /
      toggle_split() -- entering flips the flag on, AutoStateMachine owns
      the dwell timer (the policy itself gives no "done" signal, it just
      holds). At dwell end: a fair coin decides stay-in-posture (fresh
      dwell, flag stays on) vs exit. Exiting "split" is a plain flag-off
      self-rise -- splits-cycle already knows how -- followed by a settle
      pause (`posture_settle_s`, matched to SitStandCommand's ~2s ramp)
      before control moves to "walk". Exiting "sit" PREFERS the dedicated
      `standup` skill instead of sitstand's own self-rise, if --standup was
      loaded: standup's only trained entry is a genuinely seated pose,
      which only "sit" can provide, so it's chained here rather than
      pool-selectable on its own (this is what makes StandUp only ever
      trigger right after a SitStand sit). Falls back to sitstand's own
      self-rise (same settle-pause path as split) if --standup wasn't
      loaded, so "sit" still works standalone.

    - "roulade" / "kick_left" / "kick_right" / "standup" (one-shots):
      policy.trigger_behavior owns the whole lifecycle including the
      auto-return-to-walking timer; this class just watches
      policy.behavior_mode clear to know when to resume its own dwell/coin
      logic from "walk".

    Any velocity keypress (arrows/A/E/space) marks "manual" and suspends
    all of the above until `idle_timeout` seconds pass with no further
    keypress, then resumes with a fresh "walk" dwell WITHOUT clobbering
    whatever velocity was last commanded manually.
    """

    def __init__(self, policy, pool, dwell_ranges, fixed_durations,
                 vel_x_range, vel_y_range, idle_timeout, posture_settle_s):
        self.policy = policy
        self.pool = list(pool)  # e.g. ["walk","sit","split","roulade","kick_left","kick_right"]
        self.dwell_ranges = dwell_ranges          # {"walk":(min,max), "sit":(min,max), "split":(min,max)}
        self.fixed_durations = fixed_durations    # {"roulade":s, "kick_left":s, "kick_right":s, "standup":s}
        self.vel_x_range = vel_x_range
        self.vel_y_range = vel_y_range
        self.idle_timeout = idle_timeout
        self.posture_settle_s = posture_settle_s

        self._manual = False
        self._last_manual_key_time = 0.0
        self._current = None           # pool member (or one-shot name) currently active
        self._dwell_end = 0.0
        self._settling_until = None    # posture self-rise settle window, or None
        self._prev_behavior_mode = policy.behavior_mode

        self._enter_walk(time.time(), randomize=True)

    def mark_manual_key(self):
        if not self._manual:
            print("[auto] Manual control — pausing state machine "
                  f"(resumes after {self.idle_timeout:.0f}s idle)")
        self._manual = True
        self._last_manual_key_time = time.time()

    # -- entering each kind ----------------------------------------------

    def _enter_walk(self, now, randomize):
        if randomize:
            vx = random.uniform(*self.vel_x_range)
            vy = random.uniform(*self.vel_y_range)
            self.policy.set_vel_cmd(vx, vy, 0.0)
        dwell = random.uniform(*self.dwell_ranges["walk"])
        self._current = "walk"
        self._dwell_end = now + dwell
        self._settling_until = None
        print(f"[auto] WALK for {dwell:.1f}s "
              f"(vx={self.policy.vel_cmd[0]:.2f}, vy={self.policy.vel_cmd[1]:.2f})")

    def _enter_posture(self, name, now):
        flag_attr = "sit_mode" if name == "sit" else "split_mode"
        if not getattr(self.policy, flag_attr):
            (self.policy.toggle_sit if name == "sit" else self.policy.toggle_split)()
        dwell = random.uniform(*self.dwell_ranges[name])
        self._current = name
        self._dwell_end = now + dwell
        self._settling_until = None
        print(f"[auto] {name.upper()} for {dwell:.1f}s")

    def _enter_one_shot(self, name, now):
        dwell = self.fixed_durations[name]
        self.policy.trigger_behavior(name, duration=dwell)
        self._current = name
        self._dwell_end = now + dwell
        self._settling_until = None

    def _reenter(self, name, now):
        if name == "walk":
            self._enter_walk(now, randomize=True)
        elif name in _POSTURE_NAMES:
            self._enter_posture(name, now)
        else:
            self._enter_one_shot(name, now)

    # -- exiting a posture -------------------------------------------------

    def _exit_posture(self, name, now):
        if name == "sit" and "standup" in self.policy.behavior_sessions:
            # standup, not sitstand, owns this rise -- clear the flag directly
            # (NOT via toggle_sit(), which would leave sitstand's own session
            # active and let IT perform the rise before trigger_behavior ever
            # runs). No tick of sitstand-at-flag=0 happens in between.
            self.policy.sit_mode = False
            self._enter_one_shot("standup", now)
            print(f"[auto] SIT → STANDUP for {self.fixed_durations['standup']:.1f}s")
            return
        # Plain self-rise (split always; sit when --standup wasn't loaded):
        # flip the flag off, let the SAME policy perform the rise over its
        # own ramp, then wait posture_settle_s before treating it as "done".
        (self.policy.toggle_sit if name == "sit" else self.policy.toggle_split)()
        self._current = name  # still riding out the same session while it stands
        self._dwell_end = now
        self._settling_until = now + self.posture_settle_s
        print(f"[auto] {name.upper()} standing up ({self.posture_settle_s:.1f}s settle)")
    ...
    def update(self, now):
        ...
        one_shot_finished = (
            self._prev_behavior_mode in _ONE_SHOT_NAMES and behavior_mode is None
        )
        self._prev_behavior_mode = behavior_mode
        ...
        if self._manual:
            if now - self._last_manual_key_time >= self.idle_timeout:
                self._manual = False
                print("[auto] Idle — resuming automatic state machine")
                self._enter_walk(now, randomize=False)
            return

        if behavior_mode is not None:
            return  # policy.update_behavior owns this countdown

        if now < self._dwell_end:
            return

        if self._current in _POSTURE_NAMES:
            if random.random() < 0.5:
                self._enter_posture(self._current, now)  # stay seated/split, fresh dwell
            else:
                self._exit_posture(self._current, now)
            return

        # Only "walk" ever reaches here (postures handled above; one-shots
        # are intercepted by one_shot_finished before their dwell timer
        # would even matter).
        if random.random() < 0.5:
            self._reenter(self._current, now)
        else:
            choices = [m for m in self.pool if m != self._current]
            self._reenter(random.choice(choices), now)
...
def main():
    ...
    parser.add_argument("--splits-cycle", type=str, default=None,
                         help="Path to splits-cycle policy ONNX (commanded split<->stand, "
                              "flag-based like --sitstand). Requires --new-cmd-obs.")
    parser.add_argument("--standup", type=str, default=None,
                         help="Path to standup policy ONNX (rises from a SEATED pose -- only ever "
                              "triggered by the auto state machine right after a SitStand sit, since "
                              "that's its only trained entry condition). Requires --new-cmd-obs.")
    parser.add_argument("--standup-duration", type=float, default=3.0,
                         help="Seconds the standup policy stays active before handing back to walking (default: 3.0)")
    parser.add_argument("--walk-dwell", type=float, nargs=2, default=[5.0, 8.0], metavar=("MIN", "MAX"),
                         help="Auto state machine: WALK dwell time range in seconds (default: 5.0 8.0)")
    parser.add_argument("--sit-dwell", type=float, nargs=2, default=[3.0, 6.0], metavar=("MIN", "MAX"),
                         help="Auto state machine: SIT dwell time range in seconds (default: 3.0 6.0)")
    parser.add_argument("--split-dwell", type=float, nargs=2, default=[3.0, 5.0], metavar=("MIN", "MAX"),
                         help="Auto state machine: SPLIT dwell time range in seconds (default: 3.0 5.0)")
    parser.add_argument("--vel-x-range", type=float, nargs=2, default=[0.10, 0.30], metavar=("MIN", "MAX"),
                         help="Auto state machine: random forward lin_vel_x range while WALKing -- "
                              "keep positive, or the duck just stands (default: 0.10 0.30)")
    parser.add_argument("--vel-y-range", type=float, nargs=2, default=[-0.10, 0.10], metavar=("MIN", "MAX"),
                         help="Auto state machine: random lin_vel_y range (turning curvature) while "
                              "WALKing -- either sign (default: -0.10 0.10)")
    parser.add_argument("--idle-timeout", type=float, default=5.0,
                         help="Seconds of no manual velocity keypress before the auto state machine "
                              "resumes (default: 5.0)")
    parser.add_argument("--posture-settle", type=float, default=2.5,
                         help="Seconds to wait after a posture (sit/split) self-rises before treating "
                              "it as done and returning to WALK -- matched to SitStandCommand's ~2s "
                              "ramp plus buffer (default: 2.5)")
    ...
    if (args.kick_left or args.kick_right or args.roulade or args.splits_cycle or args.standup) and not args.new_cmd_obs:
        parser.error("--kick-left/--kick-right/--roulade/--splits-cycle/--standup policies use the unified 13D command obs (61D); add --new-cmd-obs")
    if (args.kick_left or args.kick_right or args.roulade or args.splits_cycle or args.standup) and args.roller:
        parser.error("kick/roulade/splits-cycle/standup policies are trained on the walking robot, not the roller model")
    if args.vel_x_range[0] <= 0:
        parser.error("--vel-x-range min must be positive, or WALK never actually walks")
    ...
    policy = PolicyInference(
        ...
        splits_cycle_onnx_path=args.splits_cycle,
        standup_onnx_path=args.standup,
    ...
    if policy.split_session:
        print(f"Splits-cycle policy: loaded  (press J to toggle)")
    ...
    if policy.walking_session:
        _pool.append("walk")
    if policy.is_sitstand:
        _pool.append("sit")
    if policy.split_session:
        _pool.append("split")
    for _name in ("roulade", "kick_left", "kick_right"):
        if _name in policy.behavior_sessions:
            _pool.append(_name)

    state_machine = None
    if "walk" in _pool and len(_pool) >= 2:
        state_machine = AutoStateMachine(
            policy,
            pool=_pool,
            dwell_ranges={
                "walk": tuple(args.walk_dwell),
                "sit": tuple(args.sit_dwell),
                "split": tuple(args.split_dwell),
            },
            fixed_durations={
                "roulade": args.roulade_duration,
                "kick_left": args.kick_duration,
                "kick_right": args.kick_duration,
                "standup": args.standup_duration,
            },
            vel_x_range=tuple(args.vel_x_range),
            vel_y_range=tuple(args.vel_y_range),
            idle_timeout=args.idle_timeout,
            posture_settle_s=args.posture_settle,
        )
        print(f"Auto state machine v2: ON  (pool: {', '.join(_pool)}"
              + (" + standup (chained after sit)" if "standup" in policy.behavior_sessions else "")
              + f"; any velocity key pauses it for {args.idle_timeout:.0f}s idle before it resumes)")
        print()
    elif _pool:
        print(f"Auto state machine v2: OFF (need --walking plus at least one more skill; "
              f"only loaded: {', '.join(_pool)})")
        print()
    ...
    def handle_key(key):
        ...
        # Anything that manipulates state AutoStateMachine also owns (vel_cmd,
        # sit/split flags, one-shot triggers) pauses it -- not just movement
        # keys. Movement keys only count when they actually reach vel_cmd
        # (head/body-pose mode intercepts them first and never touches it).
        if state_machine is not None:
            if key in ("up", "down", "left", "right", "a", "e", " ") and not policy.head_mode and not policy.body_pose_mode:
                state_machine.mark_manual_key()
            elif key in ("y", "j", "k", "l", "r"):
                state_machine.mark_manual_key()
        try:
            if key == "up":
                if policy.head_mode:
    ...
    print("  J:                toggle split (requires --splits-cycle)")
    ...
    with TerminalInput() as term, \
        ...
        try:
            ...
            while viewer.is_running() and not quit_requested:
                ...
                if state_machine is not None:
                    state_machine.update(step_start)
